(function update() {
	...
	multipler[0] *= colorenergyeff(0);
	multipler[0] /= Math.pow(10,2*(nowchallenge[0]**2));
	
	if (producer[0] * multipler[0] <= Math.pow(10,100)) {
		energy += producer[0] * multipler[0];
	}
	else {
		energy += Math.pow(producer[0] * multipler[0],0.5) * Math.pow(10,50);
	}
